/**
 * Maps browser `KeyboardEvent.code` values to the Linux evdev keycodes
 * used inside mechvibes-format soundpack config.json files
 * (e.g. KEY_BACKSPACE = 14, KEY_ENTER = 28, KEY_SPACE = 57).
 *
 * This lets a real, sample-based soundpack (downloaded from mechvibes.com
 * or its community packs) be driven correctly from a browser keydown event.
 */
...
/** Fallback "generic" bucket for keys with no exact evdev mapping */
export function evdevFor(code: string): number | null {
